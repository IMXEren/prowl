...
import prowl.browser.driver as driver_pkg
from prowl.browser import Browser as PublicBrowser
from prowl.browser import TabGroup as PublicTabGroup
from prowl.browser.browser import Browser, BrowserShutdownState, TabGroup
...
class ConcreteBoundaryShapeTests(TestCase):
    ...
    def test_invariant_no_interface_or_adapter_imports_in_concrete_files(self) -> None:
        ...
        for path in (
            ...
            "src/prowl/browser/lifecycle/startup.py",
            "src/prowl/browser/driver/runtime.py",
        ):
            src = _source(path)
            self.assertNotIn("prowl.browser.driver.interfaces", src)
            self.assertNotIn("prowl.browser.lifecycle.adapter", src)
    ...
    def test_error_path_browser_start_does_not_define_admission_callback(self) -> None:
        ...
        src = _source("src/prowl/browser/browser.py")
        tree = ast.parse(src)
...
class RuntimeStartupOrderingTests(IsolatedAsyncioTestCase):
    ...
    async def test_happy_path_launches_then_resolves_then_connects(self: Self) -> None:
        ...
        with (
            patch("prowl.browser.driver.runtime.Chrome", return_value=chrome),
            patch("prowl.browser.driver.runtime.launch_persistent_context_async", side_effect=launch),
            patch("prowl.browser.driver.runtime.resolve_cdp_ws_url", side_effect=resolve),
            patch("prowl.browser.driver.runtime.asyncio.sleep", AsyncMock()),
        ):
    ...
    async def test_error_path_resolver_failure_rolls_back_partial_runtime(self: Self) -> None:
        ...
        with (
            patch("prowl.browser.driver.runtime.Chrome", return_value=chrome),
            patch(
                "prowl.browser.driver.runtime.launch_persistent_context_async",
                AsyncMock(return_value=context),
            ),
            patch(
                "prowl.browser.driver.runtime.resolve_cdp_ws_url",
    ...
    async def test_happy_path_remote_attach_connects_without_launch_or_resolve(self: Self) -> None:
        ...
        with (
            patch("prowl.browser.driver.runtime.Chrome", return_value=chrome),
            patch("prowl.browser.driver.runtime.launch_persistent_context_async", AsyncMock()) as launch,
            patch("prowl.browser.driver.runtime.resolve_cdp_ws_url", AsyncMock()) as resolve,
        ):
    ...
    async def test_happy_path_remote_attach_initializes_playwright_and_pydoll_clients(self: Self) -> None:
        ...
        with (
            patch("prowl.browser.driver.runtime.Chrome", return_value=chrome),
            patch("prowl.browser.driver.runtime.async_playwright", return_value=playwright_owner),
            patch("prowl.browser.driver.runtime.launch_persistent_context_async", AsyncMock()) as launch,
            patch("prowl.browser.driver.runtime.resolve_cdp_ws_url", AsyncMock()) as resolve,
        ):
            await runtime.attach_remote(
    ...
    async def test_invariant_remote_connect_bypasses_local_lifecycle_hooks(self: Self) -> None:
        ...
        try:
            with (
                ...
                patch("prowl.browser.lifecycle.startup.ensure_binary") as ensure,
                patch("prowl.browser.lifecycle.startup.get_free_port") as port,
                patch("prowl.browser.driver.runtime.resolve_cdp_ws_url", new=AsyncMock()) as resolve,
