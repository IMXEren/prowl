...
class BrowserLifecycleStartupTests(IsolatedAsyncioTestCase):
    ...
    async def _start(self, *, running: bool = False) -> _StartResult:
        ...
        with (
            ...
            patch(
                "prowl.browser.lifecycle.startup.ensure_binary",
                side_effect=lambda: events.append("binary"),
            ),
    ...
    async def test_prime_profile_closes_context_when_sleep_fails(self) -> None:
        ...
        with (
            patch(
                ...
                AsyncMock(return_value=prime_ctx),
            ),
            patch(
                "prowl.browser.lifecycle.startup.asyncio.sleep",
                AsyncMock(side_effect=RuntimeError("sleep failed")),
            ),
