...
from prowl.shutdown import CoordinatorStateError, RegistrationToken
...
class ShutdownTerminalStateTests(IsolatedAsyncioTestCase):
    ...
    async def test_shutdown_unregisters_and_acknowledges_signal_once(self) -> None:
        ...
        with (
            patch("prowl.browser.lifecycle.startup.get_coordinator", return_value=coordinator),
        ):
            lc._register_signal_cleanup()

        token = coordinator.registered[0]

        with patch("prowl.browser.lifecycle.startup.get_coordinator", return_value=coordinator):
            await Browser.shutdown()
    ...
    async def test_signal_trigger_schedules_shutdown_and_returns_promptly(self) -> None:
        ...
        token = coordinator.registered[0]
        with (
            patch.object(BrowserLifecycle, "_cleanup_resources", new=AsyncMock()) as cleanup_mock,
        ):
            await Browser.shutdown()
...
class StartupRollbackTests(IsolatedAsyncioTestCase):
    ...
    async def test_prime_ctx_closed_on_sleep_failure(self) -> None:
        ...
        bases[4] = patch(
            "prowl.browser.lifecycle.startup.asyncio.sleep",
            side_effect=RuntimeError("sleep failed"),
        )
        extras = [
            patch("prowl.browser.lifecycle.startup.Path.exists", return_value=False),
            patch(
                "prowl.browser.lifecycle.startup.launch_persistent_context_async",
    ...
    async def test_startup_failure_after_shared_pd_rolls_back(self) -> None:
        ...
        bases = _startup_base_patches()
        extras = [
            patch("prowl.browser.lifecycle.startup.Path.exists", return_value=True),
            patch("prowl.browser.driver.runtime.Chrome", return_value=mock_pd),
            patch(
    ...
    async def test_startup_failure_clears_page_maps(self) -> None:
        ...
        extras = [
            ...
            patch("prowl.browser.driver.runtime.resolve_cdp_ws_url", return_value="ws://localhost:9999"),
        ]
...
class HandlerRegistrationTests(IsolatedAsyncioTestCase):
    ...
    async def test_start_registers_signal_trigger_after_success(self) -> None:
        ...
        extras = [
            patch("prowl.browser.lifecycle.startup.Path.exists", return_value=True),
            patch("prowl.browser.driver.runtime.Chrome", return_value=MagicMock(connect=AsyncMock())),
            patch(
                "prowl.browser.driver.runtime.launch_persistent_context_async",
...
class StartDuringShutdownTests(IsolatedAsyncioTestCase):
    ...
    async def test_start_after_done_ok_allows_new_generation(self) -> None:
        ...
        with (
            patch("prowl.browser.lifecycle.startup.FingerprintManager") as mock_fp,
            patch("prowl.browser.lifecycle.startup.SearchEngineInjector") as mock_sei,
            patch("prowl.browser.lifecycle.startup.get_free_port", return_value=9999),
            patch("prowl.browser.lifecycle.startup.ensure_binary"),
            patch("prowl.browser.lifecycle.startup.asyncio.sleep"),
            patch("prowl.browser.driver.runtime.Chrome") as mock_chrome,
            patch.object(BrowserLifecycle, "unpack_profile", return_value=False),
            patch("prowl.browser.lifecycle.startup.Path.exists", return_value=True),
            patch(
                "prowl.browser.driver.runtime.launch_persistent_context_async",
